...
Boot_Choice=$(zenity --width 550 --height 250 --list --radiolist --multiple --title "Clover Toolbox" --column "Select One" \
	...
	then
		...
		echo -e "$current_password\n" | sudo -S sed -i '/<key>DefaultVolume<\/key>/!b;n;c\\t\t<string>EFI<\/string>' /boot/efi/EFI/clover/config.plist
	...
	then
		...
		echo -e "$current_password\n" | sudo -S sed -i '/<key>DefaultLoader<\/key>/!b;n;c\\t\t<string>\\EFI\\fedora\\shimx64\.efi<\/string>' /boot/efi/EFI/clover/config.plist
		echo -e "$current_password\n" | sudo -S sed -i '/<key>DefaultVolume<\/key>/!b;n;c\\t\t<string>EFI<\/string>' /boot/efi/EFI/clover/config.plist
